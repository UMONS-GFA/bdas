...
if len(sys.argv) == 3:
    ...
    try :
        outfile = os.path.abspath(os.path.join(basepath, '..', 'DownloadDAS', 'CommandFiles', 'PartialDownloadDAS%s%03d.cmd' % (dasname, i)))
        cmdlines=['-%03d' % i, '#E0', '#RI', '#XP %04d %02d %02d %02d %02d %02d %04d %02d %02d %02d %02d %02d' % (oneweekago.tm_year, oneweekago.tm_mon, oneweekago.tm_mday, oneweekago.tm_hour, oneweekago.tm_min, oneweekago.tm_sec,now.tm_year, now.tm_mon, now.tm_mday, now.tm_hour, now.tm_min, now.tm_sec), '%s%03doneweek' % (dasname, i), dl_expectedduration]
