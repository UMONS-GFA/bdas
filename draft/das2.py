...
from draft import dasconnection2 as dc
from draft import station as st
from draft import sensor as ss
...
class Das(object):
    ...
    net_id = ''
    station = st.__init__()
    connection = dc.DasConnection()
    connection_type = 'none'
    sensors = []

    @abc.abstractmethod
    def __init__(self, net_id='', conn=dc.DasConnection()):

        if net_id == '':
            net_id = settings.DefaultNetid
        if conn.__class__.__name__ == 'DasConnection':
            if settings.DefaultConnectionType == 'Serial':
                conn = dc.NanoDasConnectionSerial(settings.DefaultConnectionDev)
            elif settings.DefaultConnectionType == 'TCP':
                conn = dc.NanoDasConnectionTCP(settings.LocalHost)
        self.net_id = net_id
...
class nanoDas(Das):
    ...
    def __init__(self, net_id='', conn=dc.DasConnection()):
        super(nanoDas, self).__init__(net_id='', conn=dc.DasConnection())
        if net_id == '':
            net_id = settings.DefaultNetid
        if conn.__class__.__name__ == 'DasConnection':
            if settings.DefaultConnectionType == 'Serial':
    ...
    def listen(self, timelapse):
        ...
        """
        output = ''
        print('Listening port %s' % self.net_id)
        while output == '':
    ...
    def download(self, filename, epomin='', epomax=''):  # TODO :  interruptions from WB
        ...
        if epomax != '':
            ...
            epomax = datetime.datetime.utcnow()
